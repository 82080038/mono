<?php
/**
 * Enhanced Authentication Service for Single Cooperative
        ...
        try {
            $stmt = $this->db->prepare("
                SELECT 
                    ...
                    GROUP_CONCAT(DISTINCT ur.permissions) as permissions
                FROM users u
            ...
            if ($user) {
                ...
                // Get permissions directly from single role (since each user has one role in our test setup)
                $allPermissions = [];
                if ($user['permissions']) {
                    // Parse JSON permissions directly
...
private function createSession(int $userId, array $deviceInfo = []): string {
    try {
        ...
        $expiresAt = date('Y-m-d H:i:s', time() + (Config::SESSION_TIMEOUT_MINUTES * 60));
...
private function clearFailedAttempts(string $email): void {
    // This could be implemented to clear failed attempts after successful login
}
...
public function validateToken(string $token): array {
    ...
    // Simplified logging without database dependency for now
